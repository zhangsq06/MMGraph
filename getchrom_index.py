...
parser.add_argument('--peak_flank',type=int, default=50, help='the length of the sequence')
...
def getchrom_index(args):
    ...
    file.close()
    ll = len(data1)
    #### generating bed file, fasta file
    ...
    for j in range(ll):
        ...
        t1 = t.split('\t')
        if match_res:
            t11 = round((int(t1[1]) + int(t1[2])) / 2) - args.peak_flank + 1 ## the sequence with a length of 101
...
def get_data(args):
    path = args.path
    name = args.bed  # GSE114204
    innames = [name + 'train1.fa', name + 'test1.fa'] # get the fasta file of sequences
    ...
    for i in range(2):
        ...
        path_encode = path + 'data/encode_'  + str(args.peak_flank * 2 + 1) #### data dir
