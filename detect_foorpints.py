...
parser.add_argument('--peak_flank',type=int, default=50, help='the length of peaks')
args = parser.parse_args()
#####################
def top1500(tobias_bed_name,tobias_top1500_bed_name, dirs):
    name = tobias_bed_name
    d = dirs
    file = open(name)
    da = file.readlines()
    file.close()
    new_file = open(tobias_top1500_bed_name, 'w')
    for t in da:
        t = t.strip().split('\t')
        if t[3] > d:
    ...
    new_file.close()
    f = open(tobias_top1500_bed_name)
    line = f.readlines()
    f.close()
    l = len(line)
    return l
    ...
    while l > 1510 or l < 1490:
        if l > 1510:
            ...
            if a > 0.05:
                ...
                d = str(float(d) + 0.01)
            l = top1500(tobias_bed_name,tobias_top1500_bed_name, d)
        else:
            a = (1500 - l) / 1500
            if a > 0.05:
                if a > 0.5:
