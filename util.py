...
data_names='../data/encode_tfbs.txt'
# The base-num pairs
BASE_NUM = {'A' : 1, 
            'C' : 2,
            'G' : 3,
...
path_genome = '/data/models/basset690/data/GRCh37.p13.genome.fa'                 # Fasta files of hg38
...
def load_data_encode(data_path, peak_coor, data_name,MINIMUM_POSI_SEQ=1000, path_curr_data='', train_test="train", peak_flank=50, back_grou=['gc_match']):
...
#    if not os.path.exists(path_curr_data + "/" + train_test + "/" + "sequences.npy"):
    ...
    sequences_alph = list(np.array(np.delete(sequences_alph, invalid_seq_index, 0)))
    
    all_seqs = []
    # Generate negative sequences
    if train_test != 'test':
        seqs = []
        seq_alph =[[seq].append(1) for seq in sequences_alph]
        
        while len(seqs) < len(sequences_alph):
        ...
        seq_alph_shuffle = [[seq].append(0) for seq in seqs]
    
        for i in range(len(seq_alph_shuffle)):
            all_seqs.append(seq_alph[i])
            all_seqs.append(seq_alph_shuffle[i])
    else:
        pos_seqs = sequences_alph[:int(len(sequences_alph)/2-1)]
        neg_seqs = sequences_alph[int(len(sequences_alph)/2-1):]
        pos_seqs = [ [seq].append(1) for seq in pos_seqs]
        neg_seqs = [ [seq].append(0) for seq in neg_seqs]
        for i in range(len(neg_seqs)):
            all_seqs.append(pos_seqs[i])
            all_seqs.append(neg_seqs[i])
    return all_seqs

def convert_data(index,flag='train'):
    train_data_list, table_name_list=get_data_name(data_names)
    narrowPeak='/data/models/basset690/data/TfbsUniform_hg19_ENCODE/'+table_name_list[index][:]+'.narrowPeak.gz'
    [peak_coor, peak_num] = get_peak_coor(narrowPeak)

    data_path='/data/models/basset690/data/encode_101/'+train_data_list[index]+'_AC.seq.gz'
    train_seqs = load_data_encode(data_path,peak_coor,train_data_list[index],train_test='train', peak_flank=50)
    data_path='/data/models/basset690/data/encode_101/'+train_data_list[index]+'_B.seq.gz'
    test_seqs = load_data_encode(data_path,peak_coor,train_data_list[index],train_test='test',peak_flank=50)
    print(len(train_seqs),len(test_seqs))
    return train_seqs.extend(test_seqs)

if __name__=='__main__':
    seqs = convert_data(index=0)
    print(len(seqs))
